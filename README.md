...
(Postgres, Auth, Row-Level Security). Financial data is modeled as event-based
records — transactions and valuations are stored as individual operations, and
balances and positions are derived dynamically from that history rather than
stored, so derived values stay consistent by construction.
